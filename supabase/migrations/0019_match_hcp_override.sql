-- 0019_match_hcp_override.sql
-- Per-match override pro počet bodů za správný handicap tip.
-- Když admin v adminu zadá hcp_override_points = 10, uživatelé se správným hcp tipem
-- v tomto zápasu dostanou 10 bodů místo defaultní hodnoty podle fáze (group/prelim/quarter/...).

alter table public.matches
  add column if not exists hcp_override_points integer;

comment on column public.matches.hcp_override_points is
  'Per-match override: počet bodů za správný handicap tip. null = použít default podle stage.';
...
-- Repočet všech finalized zápasů s novou logikou.
do $$
declare
  rec record;
begin
